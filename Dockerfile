...
CMD sh -c 'PGPASSWORD="$DATABASE_PASSWORD" psql -h "$DATABASE_HOST" -U "$DATABASE_USER" -d "$DATABASE_NAME" -c "SET search_path TO $SCHEMA;" && \
           echo "Starting migrations..." && \
           flask db upgrade && \
           echo "Migrations complete, starting seed..." && \
           flask seed all && \
